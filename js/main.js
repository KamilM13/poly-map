...
document.addEventListener('DOMContentLoaded', () => {
    ...
    class SceneInit {
        ...
        initScene() {
            ...
            this.camera.position.set(75, 300, 75);
    ...
    const initScene = async () => {
        let sceneInit = new SceneInit();
        sceneInit.initScene();
        sceneInit.animate();

        const polyMap = new THREE.Group();
        sceneInit.scene.add(polyMap);
      ...
      window.addEventListener('click', (event) => {
        const clickedObject = sceneInit.checkClick(event);
        if (clickedObject) {
            const systemName = clickedObject.name;
            switchToSystemScene(systemName);
        }
      });
